# Copyright 2022 the Regents of the University of California, SplatfactoEnv Team and contributors. All rights reserved.
#
...
@dataclass
class SplatfactoEnvDataParserConfig(DataParserConfig):
    """SplatfactoEnv dataset config"""

    _target: Type = field(default_factory=lambda: SplatfactoEnvDataParser)
...
@dataclass
class SplatfactoEnvDataParser(DataParser):
    """SplatfactoEnv DatasetParser"""

    config: SplatfactoEnvDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in frames:
            ...
            if "env_params" in frame:
                env_params.append(frame['env_params'])
        ...
        env_params = np.array(env_params).astype(np.float32)
        # if env_params.size > 0:
        env_params = torch.from_numpy(env_params)
        ...
        # if env_params.size > 0:
        env_params = env_params[idx_tensor]
